"""Async mirror of :mod:`unitysvc_py.resources.aliases`."""
...
class AsyncAliasesResource:
    ...
    async def set_routing(self, alias_id: str | UUID) -> Any:
        from .._generated.api.customer_aliases import customer_aliases_set_alias_routing

        return unwrap(
            await customer_aliases_set_alias_routing.asyncio_detailed(
                alias_id=UUID(str(alias_id)) if not isinstance(alias_id, UUID) else alias_id,
                client=self._client,
            )
        )
