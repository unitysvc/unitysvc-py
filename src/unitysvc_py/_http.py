...
results or :mod:`unitysvc_py.exceptions`.

Generated operations from ``openapi-python-client`` return a
``Response`` wrapper around the parsed body. The hand-written facades
in :mod:`unitysvc_py.resources` always call ``sync_detailed`` /
``asyncio_detailed`` and pass the result through :func:`unwrap`, so
callers get either a populated typed model or one of the
:class:`~unitysvc_py.exceptions.UnitysvcSDKError` subclasses.
"""
...
def unwrap(response: Any) -> Any:
    ...
    * ``content: bytes``
    * ``parsed: T | HTTPValidationError | None``
