...
def set_secret(
    ...
    async def _impl() -> tuple[str, dict[str, Any]]:
        async with async_client(api_key, base_url) as client:
            ...
            existing = model_list(await client.secrets.list(limit=1000))
            match = next((s for s in existing if s.get("name") == name), None)
            if match:
                updated = await client.secrets.update(match["id"], {"value": value})
                return ("updated", updated.to_dict() if hasattr(updated, "to_dict") else dict(updated))
            created = await client.secrets.create({"name": name, "value": value})
            return ("created", created.to_dict() if hasattr(created, "to_dict") else dict(created))
...
def delete_secret(
    ...
    async def _impl() -> dict[str, Any]:
        async with async_client(api_key, base_url) as client:
            ...
            if not match:
                raise NotFoundError(
                    f"No secret named '{name}'",
                    status_code=404,
                )
            result = await client.secrets.delete(match["id"])
