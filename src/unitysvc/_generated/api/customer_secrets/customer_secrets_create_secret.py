...
def sync_detailed(
    *,
    ...
    """ Create Secret
     ...
     Create a new secret for the current customer.

    The secret is encrypted and associated with the customer derived
    from the ``X-Role-Id`` header. **The value cannot be retrieved
    after creation** — store it securely before submitting.

    The customer's routing cache is invalidated so the gateway
    re-resolves any pending enrollments that were blocked waiting
    for this secret to exist.
